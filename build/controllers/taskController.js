...
exports.taskController = {
    ...
    `;
            if (assignee && assignee.email) {
                const notificationSent = await (0, notifications_1.sendNotification)(assignee.email, 'New Task Assigned', mailContent);
                if (!notificationSent) {
                    logger_1.logger.warn(`Failed to send notification for task ${task.id} to ${assignee.email}`);
                }
            }
            logger_1.logger.info(`New task created: ${task.id} by user: ${creatorId}`);
            res.status(201).json({ message: `New task created -taskId::${task.id} by user- userID: ${creatorId}`, task });
        }
        catch (error) {
            logger_1.logger.error('Error in task creation:', error);
    ...
    async updateTask(req, res) {
        try {
          ...
          `;
                    await (0, notifications_1.sendNotification)(newAssignee.email, 'Task Reassigned', mailContent);
                }
            }
            logger_1.logger.info(`Task updated: ${task.id} by user: ${req.user.id}`);
            res.status(201).json({ message: `Task updated: ${task.id} by user: ${req.user.id}`, task });
        }
        catch (error) {
            logger_1.logger.error('Error in task update:', error);
            res.status(500).json({ error: 'Server error' });
        }
    },
    async deleteTask(req, res) {
        try {
            ...
            await task.destroy();
            logger_1.logger.info(`Task deleted: ${id} by user: ${req.user.id}`);
            res.json({ message: 'Task deleted successfully' });
        }
        catch (error) {
            logger_1.logger.error('Error in task deletion:', error);
            res.status(500).json({ error: 'Server error' });
        }
    },
    ...
    async updateTaskStatus(req, res) {
        try {
            ...
            logger_1.logger.info(`Task ${task.id} status updated from ${oldStatus} to ${status} by user ${req.user.id}`);
            if (task.assignee && task.assignee.email) {
                await (0, notifications_1.sendNotification)(task.assignee.email, 'Task Status Updated', `Task "${task.title}" status has been updated from ${oldStatus} to ${status}`);
            }
            res.json({
            ...
            logger_1.logger.error('Error in task status update:', error);
            res.status(500).json({ error: 'Server error' });
        }
    },
};
